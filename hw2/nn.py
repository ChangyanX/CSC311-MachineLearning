...
def affine_backward(grad_y, x, w):
    ...
    #####################################################################
    # W4 slides- pg.42
    grad_x = np.dot(grad_y, w.T)   # x shape: NxM   Nxe~exM
    grad_w = np.dot(x.T, grad_y)   # w shape: Mxe   MxN~Nxe

    dz_db = np.ones((len(grad_y), 1))
    grad_b = np.dot(grad_y.T, dz_db)
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
...
def relu_backward(grad_y, x):
    ...
    for i in range(len(x)):
        for j in range(len(x[0])):
            if x[i][j] > 0.0:
                grad_x[i][j] = grad_y[i][j] * 1
            else:
                grad_x[i][j] = grad_y[i][j] * 0

    # dL_dx = dL_dy • dy_dx, where dy_dx = dE_dx
...
def nn_update(model, alpha):
    ...
    model["b1"] -= alpha * model["dE_db1"].reshape(len(model["dE_db1"]))
    model["b2"] -= alpha * model["dE_db2"].reshape(len(model["dE_db2"]))
    model["b3"] -= alpha * model["dE_db3"].reshape(len(model["dE_db3"]))
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return
...
def train(model, forward, backward, update, alpha, num_epochs, batch_size):
    ...
    # display_plot(train_ce_list, valid_ce_list, "Cross Entropy", number=0,
    #              alpha=alpha)
    # display_plot(train_acc_list, valid_acc_list, "Accuracy", number=1, alpha=alpha)
...
def main():
    ...
    # uncomment to try 5 different alphas
    # alphas = [0.001, 0.01, 0.1, 0.5, 1.]
    # for alpha in alphas:
    #     num_epochs = 1000
    #     batch_size = 100

    # uncomment to try 5 different mini-batch size, batch_size
    # batch_sizes = [10, 100, 200, 500, 1000]
    # for batch_size in batch_sizes:
    #     num_epochs = 1000
    #     alpha = 0.01

    # uncomment to try 3 different num_hiddens
    # num_hidden_list = [[4, 10], [10, 20], [80, 90], [10, 90]]
